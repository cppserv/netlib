...
int tcp_acceptext (int listen_socket, struct timeval *timeout, struct sockaddr_storage *st) {
	...
	if (timeout) {
		if (setsockopt (
		        listen_socket, SOL_SOCKET, SO_RCVTIMEO, (char *)timeout, sizeof (struct timeval)) <
		    0) {
			fputs ("setsockopt failed\n", stderr);
		}

		if (setsockopt (
		        listen_socket, SOL_SOCKET, SO_SNDTIMEO, (char *)timeout, sizeof (struct timeval)) <
		    0) {
			fputs ("setsockopt failed\n", stderr);
		}
	}
...
ssize_t tcp_message_recv (int socket, void *message, size_t len, uint8_t sync) {
	...
	do {
		...
		if (received_now > 0) {
			received += received_now;
		           ...
		           (received_now == -1 && (errno == EPIPE || errno == ENOTCONN))) {
			if (received > 0) {
				return received;
	...
	} while (received != (ssize_t)len && (sync ? ((received_now == -1 || received_now == 0) &&
 ...
 * @param idl The time (in seconds) the connection needs to remain idle before TCP starts sending
 * keepalive probes.
...
int tcp_setKeepAlive (const int socket, const int cnt, const int idl, const int intvl) {
	...
	if (WSAIoctl (
	        socket, SIO_KEEPALIVE_VALS, &keepalive_conf, sizeof (keepalive_conf), 0, 0, 0, 0, 0) !=
	    0) {
		return -2;
	}
...
#ifdef NL_DEBUG
		fprintf (
		    stderr, "Connection established with algorithm: %s\n", SSL_get_cipher_name (ret->tls));
#endif
...
SSL_CTX *getDefaultSSocketSSLconfig (enum syncSocketType mode, int freeOnError) {
	...
	if (!SSL_CTX_use_PrivateKey_file (ret, "certs/prv/worm.key.pem", SSL_FILETYPE_PEM) &&
	    freeOnError) {
		ERR_print_errors_fp (stderr);
		return NULL;
	}
...
int tcp_message_ssend (SyncSocket *socket, const void *message, size_t len) {
	if (socket->config == NULL) {
		...
		do {
			...
			} else if ((sent_now == 0 || sent_now == -1) &&
			           (errno == EPIPE || errno == ENOTCONN ||
			            SSL_get_error (socket->tls, sent_now) == SSL_ERROR_ZERO_RETURN)) {
				return -1;
			}
		} while (sent != (ssize_t)len && sent_now != -1 && sent_now != 0);
...
void *send_fun (void *args) {
	...
	for (;; current_buf = (current_buf + 1) & 1) {  // equivalent to %2
		...
		if (sock->write_pos[current_buf] > 0) {
			if (tcp_message_ssend (
			        sock->ssock, sock->buff[current_buf], sock->write_pos[current_buf]) == -1) {
				// socket closed
				pthread_spin_lock (&(sock->lock));
				sock->closed = 1;
				pthread_spin_unlock (&(sock->lock));
...
void *recv_fun (void *args) {
	...
	for (;;) {
		do {
			int received_now = tcp_message_srecv (
			    sock->ssock, sock->buff[current_buf] + received, sock->buf_len - received, 0);
...
int tcp_connect_to_async (
    char *ip, uint16_t port, AsyncSocket *sock, enum syncSocketType mode, SSL_CTX *sslConfig) {
...
int tcp_accept_async (int listen_socket,
                      AsyncSocket *sock,
                      struct timeval *timeout,
                      enum syncSocketType mode,
                      SSL_CTX *sslConfig) {
	size_t buf_len = OPTIMAL_BUFFER_SIZE;
...
int asyncSocketStartSSL (AsyncSocket *socket, enum syncSocketType mode, SSL_CTX *sslConfig) {
	...
	if (getsockopt (socket->ssock->sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&lasttimeout, NULL) <
	    0) {
		lasttimeout         = timeout;
		lasttimeout.tv_usec = 250000;
	}

	if (setsockopt (socket->ssock->sockfd,
	                SOL_SOCKET,
	                SO_RCVTIMEO,
	                (char *)&timeout,
